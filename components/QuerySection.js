...
import { handleExport, formatConversationForExport } from '../utils/exportUtils.js';
    ...
    <${CollapsibleSection} title="AI Query" headerContent=${headerContent} fullscreenOnOpen>
      ${({ isFullScreen }) => html`
        <div className=${`flex flex-col ${isFullScreen ? 'h-full' : ''}`}>
          ...
          <div className=${`bg-slate-900/50 rounded-lg p-2 flex flex-col ${isFullScreen ? 'flex-grow min-h-0' : 'h-72'}`}>
              ...
              ${isLoading && html`
                <div className="flex items-center space-x-2">
                  <div className="w-7 h-7 rounded-full bg-slate-700 flex items-center justify-center flex-shrink-0">
                    <svg className="w-4 h-4 text-teal-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth=${1.5} stroke="currentColor">
                  ...
                  <div className="bg-slate-700 p-3 rounded-lg flex items-center">
                    ...
                    <div className="w-2 h-2 bg-slate-400 rounded-full animate-pulse delay-200 mx-1"></div>
            ...
            <div className="mt-2 pt-2 border-t border-slate-700 flex-shrink-0">
                ...
                <input
                    ...
                    disabled=${isLoading}
                    className="w-full bg-slate-800 border border-slate-600 rounded-md py-1.5 px-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-teal-500 disabled:cursor-not-allowed"
                />
                <button
                    type="submit"
                    disabled=${isLoading || !query.trim()}
                    aria-label="Send question"
                    className="bg-teal-600 hover:bg-teal-500 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-bold p-2 rounded-md transition-colors duration-300"
                >
