...
import { ministryEventsData } from '../data/ministryEventsData.js';
// Fixed import path to use .js extension for ES module compatibility.
import { MinistryEvent } from '../types.js';

type Category = MinistryEvent['category'];

const CATEGORY_DETAILS: Record<Category, { label: string; color: string; border: string; }> = {
...
const TimelineCard: React.FC<{ event: MinistryEvent; index: number }> = ({ event, index }) => {
...
export const MinistryTimelineChart: React.FC = () => {
    const [activeFilters, setActiveFilters] = useState<Set<Category>>(new Set(Object.keys(CATEGORY_DETAILS) as Category[]));
            ...
            <div className="relative w-full overflow-x-auto">
                <div className="relative p-2 h-full" style={{ minWidth: '600px' }}>
                    ...
                    {filteredEvents.map((event, index) => (
                        <TimelineCard key={`${event.year}-${event.description}`} event={event} index={index} />
                    ))}
                </div>
