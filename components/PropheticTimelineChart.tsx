...
import { propheticEventsData } from '../data/propheticEventsData.js';
// Fixed import path to use .js extension for ES module compatibility.
import { PropheticEvent } from '../types.js';

type ProphecyCategory = "70 Weeks" | "Seven Times" | "Ezekiel's Siege" | "Revelation Time Periods" | "Daniel's Time Periods" | "400-Year Affliction";

const PROPHECY_DETAILS: Record<ProphecyCategory, { label: string; color: string; border: string; }> = {
...
const TimelineCard: React.FC<{ event: PropheticEvent; index: number }> = ({ event, index }) => {
    const isLeft = index % 2 === 0;
    const prophecyDetail = PROPHECY_DETAILS[event.prophecy as ProphecyCategory];
...
export const PropheticTimelineChart: React.FC = () => {
    const [activeFilters, setActiveFilters] = useState<Set<ProphecyCategory>>(new Set(Object.keys(PROPHECY_DETAILS) as ProphecyCategory[]));

    const toggleFilter = (category: ProphecyCategory) => {
    ...
    const sortedAndFilteredEvents = useMemo(() => {
        return propheticEventsData
            .filter(event => activeFilters.has(event.prophecy as ProphecyCategory))
            .sort((a, b) => {
            ...
            <div className="relative w-full overflow-x-auto">
                <div className="relative p-2 h-full" style={{ minWidth: '600px' }}>
                    ...
                    {sortedAndFilteredEvents.map((event, index) => (
                        <TimelineCard key={`${event.startYear}-${event.scriptures}`} event={event} index={index} />
                    ))}
                </div>
