...
import { apostolicEventsData } from '../data/apostolicEventsData.js';
// Fixed import path to use .js extension for ES module compatibility.
...
const CATEGORIES: ApostolicEvent['category'][] = [ 'Congregation Milestones', 'Missionary Journeys', 'Writings', 'Roman & Jewish Events' ];
const COLORS: Record<ApostolicEvent['category'], string> = { 'Congregation Milestones': 'bg-purple-500/80', 'Missionary Journeys': 'bg-sky-500/80', 'Writings': 'bg-teal-500/80', 'Roman & Jewish Events': 'bg-amber-500/80' };
const getNiceInterval = (range: number) => { if (range <= 10) return 1; if (range <= 20) return 2; if (range <= 50) return 5; return 10; };

// Function to layout events to prevent overlap within a swimlane
const layoutEvents = (events: ApostolicEvent[]) => {
    const sortedEvents = [...events].sort((a, b) => a.startYearCE - b.startYearCE);
    const layout: { event: ApostolicEvent; level: number }[] = [];
    ...
    for (const event of sortedEvents) {
        ...
        while (!placed) {
            if (levelEnds[level] === undefined || event.startYearCE >= levelEnds[level]) {
                ...
                levelEnds[level] = event.endYearCE + 1; // +1 year padding
                placed = true;
            } else {
                level++;
            }
...
export const ApostolicPeriodChart: React.FC = () => {
    ...
    const categorizedAndLaidOutData = useMemo(() => {
        const result: Record<string, { event: ApostolicEvent; level: number }[]> = {};
    ...
    const showTooltip = (e: React.MouseEvent, content: React.ReactNode) => setTooltip({ content, x: e.clientX, y: e.clientY });
    const hideTooltip = () => setTooltip(null);
    const handleResetZoom = () => setCurrentRange(initialRange);

    const pixelToYear = (pixelX: number) => {
        if (!chartRef.current) return 0;
        const { offsetWidth } = chartRef.current;
        return timelineStart + (pixelX / offsetWidth) * yearRange;
    };

    const handleMouseDown = (e: React.MouseEvent) => {
        if (e.target !== e.currentTarget.querySelector('.chart-background')) return;
        const { left } = e.currentTarget.getBoundingClientRect();
        setIsDragging(true);
        setSelection({ start: e.clientX - left, end: e.clientX - left });
    };

    const handleMouseMove = (e: React.MouseEvent) => {
        if (!isDragging || !chartRef.current) return;
        const { left } = e.currentTarget.getBoundingClientRect();
        setSelection(prev => ({ ...prev, end: e.clientX - left }));
    };
    ...
    const handleMouseUp = () => {
        ...
        if (selection.start !== null && selection.end !== null && Math.abs(selection.start - selection.end) > 10) {
            ...
            setCurrentRange({ start: Math.min(y1, y2), end: Math.max(y1, y2) });
        }
    ...
    return (
        <div className="relative w-full overflow-x-auto bg-slate-900/50 p-4 rounded-lg border border-slate-700">
            {isZoomed && <button onClick={handleResetZoom} className="absolute top-2 right-2 z-30 bg-slate-700 hover:bg-slate-600 text-teal-300 text-xs py-1 px-3 rounded-full">Reset Zoom</button>}
            <div className="flex">
                <div className="w-40 flex-shrink-0 text-right pr-4 text-xs font-bold text-slate-400 pt-[52px]">
                    {CATEGORIES.map(cat => <div key={cat} className="h-20 flex items-center justify-end">{cat}</div>)}
                </div>
                <div 
                    ref={chartRef} 
                    className="flex-grow relative cursor-crosshair" style={{ minWidth: '1000px' }}
                    onMouseUp={handleMouseUp} onMouseLeave={() => isDragging && handleMouseUp()}
                >
                    <div className="absolute top-0 left-0 w-full h-full chart-background" onMouseDown={handleMouseDown} onMouseMove={handleMouseMove}/>
                    <div className="relative h-6 mb-2">
                        {yearMarkers.map(year => {
                            const percent = ((year - timelineStart) / yearRange) * 100;
                            return (percent >= 0 && percent <= 100) && (
                                <div key={year} className="absolute h-full top-0" style={{ left: `${percent}%` }}>
                    ...
                    <div className="relative">
                        {yearMarkers.map(year => {
                            const percent = ((year - timelineStart) / yearRange) * 100;
                             return (percent >= 0 && percent <= 100) && (
                                <div key={`line-${year}`} className="absolute h-full top-0" style={{ left: `${percent}%`}}>
                        ...
                        {CATEGORIES.map((category) => (
                            <div key={category} className="h-20 relative border-t border-slate-800">
                                {categorizedAndLaidOutData[category].map(({ event, level }) => {
                                    const leftPercent = ((event.startYearCE - timelineStart) / yearRange) * 100;
                                    const end = Math.max(event.startYearCE + 0.5, event.endYearCE); // ensure minimum width for single-year events
                                    const widthPercent = Math.max(0.2, ((end - event.startYearCE) / yearRange) * 100);
                                    
                                    const yOffset = level * 12; // vertical offset for overlapping events
                    ...
                    {isDragging && selection.start !== null && selection.end !== null && (
                        <div className="absolute top-0 h-full bg-teal-500/20 border-2 border-teal-400 pointer-events-none z-20" style={{
