import { useMemo, useState, useRef, createElement, Fragment } from 'react';
import htm from 'htm';
...
export const ReignsAndProphetsChart = ({ significantEvents }) => {
    ...
    const processedEvents = useMemo(() => {
        ...
        const avgCharHeightInYears = (yearRange / (chartRef.current?.offsetWidth || 1400)) * 7;
    ...
    return (
        ...
        <div className="relative w-full overflow-x-auto bg-slate-900/50 p-4 rounded-lg border border-slate-700/50">
            ...
            >
                ...
                ${data.map((group) => {
                    const chartWidthPx = chartRef.current?.offsetWidth || 1400;
                    ...
                    const totalWidthPx = (totalWidthPercent / 100) * chartWidthPx;
                                ...
                                return (
                                    ...
                                    <${Fragment} key=${`${person.type}-${person.startYearBCE}`}>
                                        <div
                                            ...
                                            onMouseMove=${(e) => showTooltip(e, tooltipContent(person))}
                                        ...
                                        ${person.embeddedProphets?.map((prophetName, index, arr) => {
                                            ...
                                            return (
                                                ...
                                                <div 
                                                    ...
                                                    onMouseMove=${(e) => {
                                                        e.stopPropagation();
                                                        showTooltip(e, html`<div class="font-bold text-yellow-300">${prophetName}</div>`)
                                                    }}
                                                    onMouseLeave=${hideTooltip}
                                                >
