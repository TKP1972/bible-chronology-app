...
const TimelineCard: React.FC<{ entry: BeliefsClarifiedEntry; index: number }> = ({ entry, index }) => {
            ...
            <div className={`order-1 bg-amber-800/80 rounded-lg shadow-xl flex-1 px-4 py-3 timeline-item-animate`}>
                ...
                <ul className="list-disc list-inside mt-2 space-y-1.5">
                    {entry.events.map((eventText, i) => (
                        <li key={i} className="text-xs text-slate-200 leading-snug">{eventText}</li>
                    ))}
                </ul>
            </div>
        </div>
...
export const BeliefsClarifiedTimelineChart: React.FC = () => {
    return (
