...
import { handleExport, formatAnalysisForExport } from '../utils/exportUtils.js';
  ...
  const headerContent = (
    ...
    <div className="flex items-center space-x-4">
      ...
      <button
        ...
        className="bg-teal-600 hover:bg-teal-500 disabled:bg-slate-600 disabled:cursor-not-allowed text-white font-bold py-1 px-3 rounded-md transition-all duration-300 ease-in-out shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-opacity-75 text-sm"
      >
        ...
        <div className=${`flex flex-col ${isFullScreen ? 'h-full' : ''}`}>
          ...
          ${analysis && html`
            <div className=${`mt-4 max-w-none ${isFullScreen ? 'flex-grow min-h-0' : ''}`}>
              <div className=${`p-3 bg-slate-900/50 rounded-lg ${isFullScreen ? 'h-full overflow-y-auto' : 'max-h-96 overflow-y-auto'}`}>
